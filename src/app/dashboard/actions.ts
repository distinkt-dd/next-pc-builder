...
> {
	...
	if (componentIds.length === 0) {
		return { success: false, error: 'Добавьте хотябы один компонент в ПК' }
	}
	...
	if (components.length !== componentIds.length) {
		return { success: false, error: 'Некторые компоненты не найдены!' }
	}
	...
	try {
		...
		return { success: false, error: 'Неудалось сохранить сборку!' }
	}
}
